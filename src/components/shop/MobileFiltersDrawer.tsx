...
export function MobileFiltersDrawer({
  ...
  const drawer = open
    ? createPortal(
        <>
          <div className="fixed inset-0 z-[140] backdrop-blur-sm" style={{ background: 'rgba(10,31,68,0.55)' }} onClick={() => setOpen(false)} />

          <div
            ref={drawerRef}
            className="fixed inset-y-0 end-0 z-[141] w-[88vw] max-w-sm shadow-2xl"
            ...
            <div className="flex h-full flex-col">
              <div className="border-b px-5 py-4" style={{ borderColor: 'var(--line)' }}>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-[0.18em]" style={{ color: 'var(--orange)' }}>
                      {t('Refine results', 'تحسين النتائج')}
                    </p>
                    <h2 className="mt-1 text-lg font-black" style={{ color: 'var(--ink)' }}>
                      {t('Filters', 'الفلاتر')}
                    </h2>
                  </div>
                  <button
                    onClick={() => setOpen(false)}
                    className="flex h-10 w-10 items-center justify-center rounded-2xl border bg-white transition-colors hover:opacity-75"
                ...
                {activeCount > 0 && (
                  <div className="mt-3 inline-flex rounded-full px-3 py-1 text-xs font-black" style={{ background: 'rgba(255,107,44,0.12)', color: 'var(--orange)' }}>
                      ...
                      {categories.map((category) => {
                        ...
                        return (
                          <button
                            ...
                            className="w-full rounded-2xl border px-4 py-3 text-start text-sm font-black transition-all"
                            style={{
                              paddingInlineStart: `${16 + ((category.level ?? 0) * 18)}px`,
                              ...(active
                                ? { borderColor: 'var(--orange)', background: 'var(--orange)', color: '#fff' }
                                : { borderColor: 'var(--line)', background: '#fff', color: 'var(--ink)' })
                            }}
              ...
              <div className="border-t px-5 py-4" style={{ borderColor: 'var(--line)' }}>
                <div className="flex gap-3">
                  {hasFilters && (
                    <button
                      onClick={clearAll}
                      className="flex-1 rounded-2xl border bg-white px-4 py-3 text-sm font-black transition-colors hover:border-rose-200 hover:text-rose-600"
                      style={{ borderColor: 'var(--line)', color: 'var(--mute)' }}
                    >
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 rounded-2xl border bg-white px-4 py-2.5 text-sm font-black shadow-sm transition-all hover:opacity-80 lg:hidden"
      ...
      >
        ...
        {t('Filters', 'الفلاتر')}
        {activeCount > 0 && (
          <span className="flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-black text-white" style={{ background: 'var(--orange)' }}>
            {activeCount}
          </span>
        )}
      </button>
...
function FilterSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="mb-6">
      <h3 className="mb-3 text-xs font-black uppercase tracking-[0.18em]" style={{ color: 'var(--mute)' }}>{title}</h3>
      {children}
    </section>
  )
}
